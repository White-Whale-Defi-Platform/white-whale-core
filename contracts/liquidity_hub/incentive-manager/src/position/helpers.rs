use cosmwasm_std::{to_json_binary, Decimal256, Deps, Env, MessageInfo, Uint128, WasmMsg};
...
pub fn calculate_weight(params: &PositionParams) -> Result<Uint128, ContractError> {
    if !(SECONDS_IN_DAY..=SECONDS_IN_YEAR).contains(&params.unbonding_duration) {
    ...
    let amount_uint = params.lp_asset.amount;
    ...
    let unbonding_duration = Decimal256::from_atomics(params.unbonding_duration, 0).unwrap();
    let amount = Decimal256::from_atomics(params.lp_asset.amount, 0).unwrap();
