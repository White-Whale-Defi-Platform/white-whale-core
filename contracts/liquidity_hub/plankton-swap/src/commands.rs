...
pub mod swap {
    ...
    use crate::{helpers, state::{get_decimals, store_fee, COLLECTABLE_PROTOCOL_FEES, TOTAL_COLLECTED_PROTOCOL_FEES, ALL_TIME_BURNED_FEES}};
...
pub mod liquidity {
    ...
    pub fn provide_liquidity(
        ...
        let collected_protocol_fees =
            COLLECTABLE_PROTOCOL_FEES.load(deps.storage, &pair_info.liquidity_token.to_string()).unwrap_or(vec![]);
    ...
    pub fn withdraw_liquidity(
        ...
        let total_share = get_total_share(&deps.as_ref(), liquidity_token.clone())?;
        let collected_protocol_fees = COLLECTABLE_PROTOCOL_FEES.load(deps.storage, &liquidity_token.to_string())?;
