use cosmwasm_std::{Addr, BankMsg, coin, coins, CosmosMsg, Decimal, DepsMut, Env, from_binary, MessageInfo, OverflowError, Response, StdError, StdResult, to_binary, Uint128, WasmMsg};
use cw20::{Cw20ExecuteMsg, Cw20ReceiveMsg};

use terraswap::asset::{Asset, AssetInfo, MINIMUM_LIQUIDITY_AMOUNT, PairInfoRaw};
...
use crate::state::{
    ALL_TIME_COLLECTED_PROTOCOL_FEES, COLLECTED_PROTOCOL_FEES, CONFIG, PAIR_INFO,
    store_protocol_fee,
};
...
pub fn swap(
    ...
    if !swap_computation.burn_fee_amount.is_zero() {
        ...
        let burn_msg = match burn_asset.info {
            AssetInfo::Token { contract_addr } => {
                CosmosMsg::Wasm(WasmMsg::Execute {
                    contract_addr,
                    msg: to_binary(&Cw20ExecuteMsg::Burn { amount: burn_asset.amount })?,
                    funds: vec![],
                })
            }
            AssetInfo::NativeToken { denom } => {
                CosmosMsg::Bank(BankMsg::Burn { amount: coins(burn_asset.amount.u128(), denom) })
            }
        };
