...
pub fn swap(
    ...
    let collect_fees_in = config.collect_protocol_fees_in;
    ...
    helpers::assert_max_spread(
        ...
        max_spread,
        offer_asset.clone(),
        swap_computation.clone().return_asset,
    ...
    let mut messages: Vec<CosmosMsg> = vec![];
    if !swap_computation.return_asset.amount.is_zero() {
        messages.push(
            swap_computation
                .clone()
                .return_asset
    ...
    if !swap_computation.burn_fee_asset.amount.is_zero() {
        store_fee(
            deps.storage,
            swap_computation.burn_fee_asset.amount,
            swap_computation.clone().burn_fee_asset.get_id(),
            ALL_TIME_BURNED_FEES,
        )?;

        messages.push(swap_computation.clone().burn_fee_asset.into_burn_msg()?);
    }
    ...
    store_fee(
        deps.storage,
        swap_computation.protocol_fee_asset.amount,
        swap_computation.clone().protocol_fee_asset.get_id(),
    ...
    Ok(Response::new().add_messages(messages).add_attributes(vec![
        ...
        (
            "return_amount",
            &swap_computation.return_asset.amount.to_string(),
        ),
        ("spread_amount", &swap_computation.spread_amount.to_string()),
        (
            "swap_fee_amount",
            &swap_computation.swap_fee_asset.amount.to_string(),
        ),
        (
            "burn_fee_amount",
            &swap_computation.burn_fee_asset.amount.to_string(),
        ),
        (
            "protocol_fee_amount",
            &swap_computation.protocol_fee_asset.amount.to_string(),
        ),
        (
            "collected_fees_in",
            &swap_computation.protocol_fee_asset.info.to_string(),
        ),
    ]))
}
