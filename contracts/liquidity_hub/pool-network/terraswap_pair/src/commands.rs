...
pub fn swap(
    ...
    if !swap_computation.osmosis_fee_amount.is_zero() {
        let osmosis_fee_asset = Asset {
            info: ask_pool.info.clone(),
            amount: swap_computation.osmosis_fee_amount,
        };

        // send osmosis fee to the osmosis fee collector
        messages.push(osmosis_fee_asset.into_msg(config.osmosis_fee_collector_addr)?);
    }
