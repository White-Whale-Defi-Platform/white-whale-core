...
use cosmwasm_std::{
    ...
    MessageInfo, OverflowError, Reply, ReplyOn, Response, StdError, StdResult, SubMsg, Uint128,
    WasmMsg,
};
...
pub fn swap(
    ...
    let asset_id = get_asset_id(ask_pool.clone().info);
    store_protocol_fee(deps.storage, swap_computation.protocol_fee_amount, asset_id)?;
